...
<body>
	...
	<section class="all_body_section">
		...
		<p id="map_cata_show">이미지로 편의시설을 찾아볼 수 있습니다</p>
		<p id="map_loc_show">위치를 검색할 수 있는 창이 열립니다</p>
		<p id="map_marker_show">마커를 사용할 수 있습니다.</p>
		<div class="map_control_btn" id="map_controller">
			<button onclick="searchPlaces('location')" class="btn_search"
				id="btn_search_loc">위치 검색하기</button>
			<button onclick="function_cata('catagory')" class="btn_search"
				id="btn_search_cata">카테고리 검색하기</button>
			<button onclick="function_dis('distance')" class="btn_search"
				id="btn_search_distance">거리 측정하기</button>
			<button onclick="function_marker('map_marker')" class="btn_search"
				id="btn_search_marker">마커사용하기</button>
		</div>
		<div class="map_wrap">
			<div id="map"></div>
			<!--################로드뷰 숨김######################
	...
	<script>
		...
		function displayPlaces(places) {
			...
			for (var i = 0; i < places.length; i++) {
						...
						// 마커를 생성하고 지도에 표시합니다
						placePosition = new kakao.maps.LatLng(places[i].y,
								places[i].x), markerPlace = addMarker(
		...
		function function_cata() {
			...
			function searchPlacesA() {
				...
				psd.categorySearch(currCategory, placesSearchCB, {
					useMapBounds : true
				});
			}
			...
			function displayPlacesA(places) {
				...
				for (var i = 0; i < places.length; i++) {
					...
					// 마커를 생성하고 지도에 표시합니다
					var markerConvenient = addMarker(new kakao.maps.LatLng(
			...
			function addMarkerA(position, order) {
				...
				// 마커 좌표에 일치시킬 이미지 내에서의 좌표
				}, markerImageA = new kakao.maps.MarkerImage(imageSrc,
						imageSize, imgOptions), markerA = new kakao.maps.Marker(
						{
							position : position, // 마커의 위치
							image : markerImageA
						});
